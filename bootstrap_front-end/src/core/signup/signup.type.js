import { SIGNUP_FORM_FIELD_KEY, SIGNUP_TITLE_KEY } from './frame/signup-form';

export const SIGNUP_FIELD_NAME = {
  LOGIN: 'login',
  PASSWORD: 'password',
};

export const SIGNUP_FORM_FIELD_NAME = {
  [SIGNUP_FORM_FIELD_KEY.LOGIN]: SIGNUP_FIELD_NAME.LOGIN,
  [SIGNUP_FORM_FIELD_KEY.PASSWORD]: SIGNUP_FIELD_NAME.PASSWORD,
};


export const SIGNUP_ACTION_TYPE = { 
  SIGNUP_FORM_UPLOAD_PENDING: "SIGNUP_ACTION_TYPE.SIGNUP_FORM_UPLOAD_PENDING",
  SIGNUP_FORM_UPLOAD_SUCCESS: "SIGNUP_ACTION_TYPE.SIGNUP_FORM_UPLOAD_SUCCESS",
  SIGNUP_FORM_UPLOAD_ERROR: "SIGNUP_ACTION_TYPE.SIGNUP_FORM_UPLOAD_ERROR",
  // SIGNUP_FORM_UPLOAD_RESET: "AUTH_FORM_UPLOAD_LOADING.SIGNUP_FORM_UPLOAD_RESET"
}

export const SIGNUP_DATA_NAME = {
  LOGIN: 'login',
  PASSWORD: 'password',
}

export const SIGNUP_TITLE_NAME = {
  REGISTRATION: 'Регистрация',
  ACCOUNT: 'Есть аккаунт? Вход',
  LOADING: 'Загружаю....',
  CREATE_BUTTON_NAME: 'Создать аккаунт'
}

export const SIGNUP_TITLE_FIELD_NAME = {
  [SIGNUP_TITLE_KEY.REGISTRATION]: SIGNUP_TITLE_NAME.REGISTRATION,
  [SIGNUP_TITLE_KEY.ACCOUNT]: SIGNUP_TITLE_NAME.ACCOUNT,
  [SIGNUP_TITLE_KEY.LOADING]: SIGNUP_TITLE_NAME.LOADING,
  [SIGNUP_TITLE_KEY.CREATE_BUTTON_NAME]: SIGNUP_TITLE_NAME.CREATE_BUTTON_NAME,
};
